...
data ValiGen s t a b =
  ValiGen { vgApply :: s -> t
          , vgGen :: a -> Gen b
          }

value :: t -> ValiGen s t a t
value x = ValiGen (const x) (const (pure x))

type Opt = Alt Maybe

-- | Disjunction
instance Semigroup t => Semigroup (ValiGen s t a b) where
  ValiGen f g <> ValiGen f' g' =
    ValiGen (f <> f') (\x -> oneof [g x, g' x])

instance Monoid t => Monoid (ValiGen s t a b) where
  mempty = ValiGen mempty (const (oneof []))

-- | Composition
(%) :: ValiGen s t a b -> ValiGen u s x a -> ValiGen u t x b
(%) (ValiGen f g) (ValiGen f' g') =
  ValiGen (f . f') (g <=< g')
...
total :: ValiGen s (Opt t) a b -> ValiGen s t a b
total vg =
  ValiGen
